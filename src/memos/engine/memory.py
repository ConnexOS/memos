...
class ContextMemory:
    def __init__(self, collection_name: str = None, store: VectorStore = None):
        ...
        self._bm25_building = False  # v0.4.4 P0-1: 构建中标记，防止增量写入半初始化索引
        self._vacuum_lock = threading.Lock()  # v0.4.0 HIGH-2: VACUUM 期间防止并发写入
    ...
    def _ensure_bm25_index(self):
        ...
        with self._bm25_lock:
            ...
            try:
                all_docs = self.store.get(where={"active": {"$ne": False}})["documents"]
                if not all_docs:
                    self._bm25 = None
                    self._bm25_docs = []
                    return
    ...
    def recall(
        ...
        if needs_rerank:
            ...
            for i, (doc, meta, dist) in enumerate(zip(docs, metadatas, distances)):
                ...
                score *= decay_factor * (1 + reuse_boost)
    ...
    def list_memories(
        ...
        all_meta = self.store.get(where=where_clause, include=["metadatas"])
    ...
    def list_todos(
        ...
        按 sort_order 升序排列。
        """
        where_clauses = [{"type": "todo"}, {"active": {"$ne": False}}]
        if todo_status:
    ...
    def export_memories(
        ...
        review_status: "approved" | "pending"，按复审状态过滤。
        memory_ids: 指定记忆 ID 列表，优先级最高，传入后忽略 type_filter/since/until/review_status。
        """
        if memory_ids:
        ...
        # review_status 过滤
        if review_status:
            and_clauses.append({"review_status": review_status})
        ...
        while True:
            ...
            # 在内存中应用额外的过滤条件（since/until/review_status——这些字段 ChromaDB where 不完全支持）
            if since or until or review_status:
                filtered = []
                for item in batch:
    ...
    def archive_memory(self, mem_id: str):
        old = self.get_memory(mem_id)
        if old is None:
            raise ChromaDBError(f"记忆未找到: {mem_id[:8]}...", detail=f"id={mem_id}")
        self.store.update(ids=[mem_id], metadatas=[{"active": False}])
        logger.info("archived: %s...", mem_id[:8])

    def restore_memory(self, mem_id: str):
        old = self.get_memory(mem_id)
        if old is None:
            raise ChromaDBError(f"记忆未找到: {mem_id[:8]}...", detail=f"id={mem_id}")
        self.store.update(ids=[mem_id], metadatas=[{"active": True}])
    ...
    def archive_old_memories(self, days: int = None) -> int:
        ...
        results = self.store.get(where={"$and": [{"active": {"$ne": False}}, {"timestamp": {"$lte": cutoff}}]})
        ids = results["ids"]
        if not ids:
            logger.info("no memories older than %d days to archive", days)
            return 0
        self.store.update(ids=ids, metadatas=[{"active": False}] * len(ids))
        logger.info("archived %d memories older than %d days", len(ids), days)
        return len(ids)

    def get_expiry_status(self) -> dict:
        """获取过期状态统计：即将过期和已过期的记忆数量。"""
        now = time.time()
    ...
    def _compute_reuse_boost(self, meta: dict, now: float = None) -> float:
        ...
        if reuse_count <= 0:
            return 0.0
        return math.log2(reuse_count + 1) * 0.15
