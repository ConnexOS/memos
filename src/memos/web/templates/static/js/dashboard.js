...
// --- 工具函数 ---
// [nav-modals] 已迁移至 static/js/nav-modals.js — 修改时请同步更新两端
function timeAgo(ts) {
...
// [nav-modals] escapeHtml 已迁移至 static/js/nav-modals.js
...
// v0.7.2: 提炼结果卡片类型标签渲染（新 4 类展示彩色 badge，旧类型不展示）
function renderTypeBadge(type) {
    const badges = {
        'solution': { label: '方案', cls: 'bg-success' },
        'decision': { label: '决策', cls: 'bg-primary' },
        'lesson': { label: '经验', cls: 'bg-warning text-dark' },
        'process': { label: '流程', cls: 'bg-info text-dark' },
    };
    const t = badges[type];
    if (t) return `<span class="badge ${t.cls}">${t.label}</span>`;
    return '';
}
...
// [nav-modals] _safeErrMsg / api 已迁移至 static/js/nav-modals.js
                ...
                <div class="card-body py-2 px-3">
                    <div class="d-flex justify-content-between align-items-start mb-1">
                        <div class="d-flex gap-1 align-items-center flex-wrap">
                            ...
                            <code class="fw-bold text-white" style="font-size:.7rem;font-family:monospace;">${escapeHtml(m.id)}</code>
                        </div>
...
document.getElementById('project-selector')?.addEventListener('change', function() {
    ...
    // 刷新今日回顾右侧栏：日报历史列表（项目隔离）
    setTimeout(function() { loadDailyReviewSidebar(); }, 150);
    // 刷新统计图表
...
// 任务审计：日期选择自动触发刷新
document.getElementById('audit-date')?.addEventListener('change', function() {
    loadTaskAudit();
});
...
// [nav-modals] 备份管理已迁移至 static/js/nav-modals.js
...
// [nav-modals] sectionLabels/sectionDescs 已迁移至 nav-modals.js

// [nav-modals] 配置管理/LLM端点/设置/语言切换已迁移至 static/js/nav-modals.js
...
document.getElementById('daily-review-tab')?.addEventListener('shown.bs.tab', async function() {
    ...
    loadDailyReviewSidebar();  // 加载日报历史列表
});

// 今日回顾右侧栏：加载历史日报列表
async function loadDailyReviewSidebar() {
    try {
        const data = await apiClient.request('/api/conversations/daily-review/list');
        const sidebar = document.getElementById('daily-review-sidebar-list');
        if (!sidebar) return;
        sidebar.innerHTML = (data.files || []).map(function(f) {
            const date = (f.name || '').replace('.md', '');
            const preview = f.preview || '';
            return '<button class="list-group-item list-group-item-action py-1 small" onclick="loadDailyReview(\'' + f.name + '\')">' +
                '<span class="fw-bold">' + date + '</span>' +
                '<div class="text-muted text-truncate" style="font-size:0.75rem;">' + preview + '</div>' +
                '</button>';
        }).join('');
        if (!data.files || !data.files.length) {
            sidebar.innerHTML = '<div class="text-muted small">暂无历史日报</div>';
        }
    } catch (e) {
        console.error('日报列表加载失败:', e);
    }
}

// 加载指定历史日报的完整内容
async function loadDailyReview(filename) {
    try {
        const data = await apiClient.request('/api/conversations/daily-review/get?file=' + encodeURIComponent(filename));
        // 显示在左侧内容区，隐藏空状态和 loading
        document.getElementById('dr-empty').classList.add('d-none');
        document.getElementById('dr-loading').classList.add('d-none');
        document.getElementById('dr-error').classList.add('d-none');
        var container = document.getElementById('dr-report-container');
        container.classList.remove('d-none');
        document.getElementById('dr-report-content').innerHTML = '<div class="p-2">' + renderMarkdown(data.content) + '</div>';
        document.getElementById('dr-info').textContent = '历史日报: ' + (data.date || filename);
        // 禁用生成相关按钮，仅保留复制
        ['dr-preview-btn','dr-save-btn','dr-regenerate-btn','dr-extract-todos-btn'].forEach(function(id){
            document.getElementById(id).disabled = true;
        });
        document.getElementById('dr-copy-btn').disabled = false;
    } catch (e) {
        console.error('加载日报失败:', e);
        var errEl = document.getElementById('dr-error');
...
// [nav-modals] 项目管理已迁移至 static/js/nav-modals.js
...
async function loadTaskAudit() {
    ...
    const data = await apiClient.request(`/api/tasks/audit?date=${date}`);
    const timeline = document.getElementById('audit-timeline');

    if (!data.items?.length) {
        timeline.innerHTML = '<div class="text-center text-muted py-3">该日无任务记录</div>';
        return;
    }
    ...
    timeline.innerHTML = data.items.map(item => {
        ...
        // 暖色调时段：[9,12) 上午工作 & [14,18) 下午工作
        const isWarm = (hour >= 9 && hour < 12) || (hour >= 14 && hour < 18);
        const titleClass = isWarm ? 'text-warning' : 'text-primary';
        const doneClass = isWarm ? 'text-warning-emphasis' : 'text-success';
        // 复用活跃任务面板的 parseTaskEval 统一解析
        const parsed = parseTaskEval({ document: item.document });
        const goal = item.goal || parsed.goal || '（无目标）';
        const doneHtml = (parsed.done || []).map(d => `<div class="ms-3 small ${doneClass}">├─ ✅ ${escapeHtml(d)}</div>`).join('');
        return `<div class="mb-2">
            <div class="fw-bold small ${titleClass}">● ${time} — ${escapeHtml(goal)}</div>
            ${doneHtml || '<div class="ms-3 small text-muted">（该日任务无已完成项）</div>'}
        </div>`;
    }).join('');
}
...
// v0.7.2: URL hash 记忆高亮支持 (#tab=knowledge&highlight=<memory_id>)
(function() {
    var hash = window.location.hash;
    if (hash.startsWith('#tab=knowledge&highlight=')) {
        var memoryId = hash.split('=')[2];
        // 切换到记忆管理 tab
        document.getElementById('tab-memory')?.click();
        // 高亮记忆（3s 超时降级）
        var highlightTimer = setTimeout(function() {
            // 降级：按 memory_id 筛选
            var filterInput = document.getElementById('mm-search-query');
            if (filterInput) filterInput.value = memoryId;
        }, 3000);

        // 尝试 scrollIntoView 高亮
        var el = document.getElementById('memory-' + memoryId);
        if (el) {
            clearTimeout(highlightTimer);
            el.scrollIntoView({ behavior: 'smooth', block: 'center' });
            el.classList.add('memory-highlight');
            setTimeout(function() { el.classList.remove('memory-highlight'); }, 2000);
        }
    }
})();
